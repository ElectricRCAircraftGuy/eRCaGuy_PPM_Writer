...
// #define TC_USE_TIMER0 //8-bit, normally controls micros(), millis(), etc <--use if you don't want any conflict with the servo library, and you are going to use eRCaGuy_TimerCounter in place of micros() and millis() 
#define TC_USE_TIMER1 //16-bit, used by the servo library, etc  <--use as the default for eRCaGuy_TimerCounter
// #define TC_USE_TIMER2 //8-bit, used by some libraries, including the core Arduino tone() function <--use if you don't want any conflict with the servo library, and you still want to have access to micros() and millis()
