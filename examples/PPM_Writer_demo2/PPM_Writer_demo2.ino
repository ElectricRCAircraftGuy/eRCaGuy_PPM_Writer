...
void setup()
{
    ...
    // Currently, you cannot set above 65535 since Timer1 is a 16-bit timer/counter, and I'm not
    // taking into account rollovers past this value, *yet*; NB: the UL is *mandatory* to force the
    // constant/literal multiplication here to follow unsigned long rules *instead of* signed int
    // rules, which I believe are default otherwise here in this compiler's C++ handling; take out
    // the UL and you'll see weird results/errors for any values where the result of the
    // multiplication is > 32767, which is the max value storable in a signed int.
    PPMWriter.setPPMPeriod(20000 * 2UL);
    Serial.print(F("PPMPeriod(0.5us) = "));
    Serial.println(PPMWriter.getPPMPeriod());
    Serial.print(F("PPMFreq(Hz) = "));
    Serial.println(PPMWriter.getPPMFrequency());
...
void loop()
{
    ...
    if (PPMWriter.readChannelFlag(ch_i) == true)
    {
        ...
        Serial.print(F("ch_i = "));
        Serial.print(ch_i);
        Serial.print(F(", frameNum = "));
        Serial.print(PPMWriter.getFrameNumber());
        Serial.print(F(", t_now(us) = "));
        Serial.print(t_now);
        Serial.print(F(", t_now2(us) = "));
        Serial.print(t_now2);
        Serial.print(F(", count_now(0.5us) = "));
        Serial.print(count_now);
        // deltas:
        Serial.print(F(", delta times = "));
        Serial.print(t_now - t_now_old);
        Serial.print(", ");
        Serial.print(t_now2 - t_now2_old);
        Serial.print(", ");
        Serial.println(count_now - count_now_old);
